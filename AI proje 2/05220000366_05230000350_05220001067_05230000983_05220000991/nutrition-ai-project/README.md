...
│   ├── meal_planner.py     # Legacy random-search prototype, not used in current app flow
│   └── visualization.py
│
├── notebooks/
│   ├── data_analysis.ipynb
...
- Machine learning based weekly weight change prediction
- Rule-based diet recommendation
- Genetic algorithm based meal plan optimization
- Allergy and cuisine filtering
- Result visualization with Plotly charts
- CSV output generation for model results and generated meal plans

## Note About meal_planner.py

meal_planner.py was used in the early prototype stage for simple random-search based meal plan generation. In the current version, active meal planning is handled by genetic_algorithm.py. The file is kept in the project to document the previous approach and to show the improvement from random search to genetic algorithm based optimization.
